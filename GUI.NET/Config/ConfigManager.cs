...
namespace Mesen.GUI.Config
{
	class ConfigManager
	{
		...
		public static string DefaultDocumentsFolder
		{
			...
			{
				if(Program.IsMono) {
					return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), ".config", "mesen-m");
				} else {
					return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Mesen-M");
				}
			}
		}
		...
		public static void InitHomeFolder()
		{
			...
			string legacyPortableFolder = Path.Combine(Path.GetDirectoryName(Assembly.GetEntryAssembly().Location), "Mesen-M");
			string documentsFolder = DefaultDocumentsFolder;
			
			//Linux only
			string legacyDocumentsFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Mesen-M");
